...
#adj_mat = nx.to_scipy_sparse_matrix(GRAPH_a, nodelist=None, dtype=None, weight='unweighted', format='csr')
...
#print(sc.affinity_matrix_.shape)
...
# # kappa affinity propagation clustering GRAPH_b

# A = nx.to_scipy_sparse_matrix(GRAPH_b, weight='unweighted', format='csr')

# A_A = A.toarray()

# ap = AffinityPropagation(affinity='precomputed').fit(A_A)
# labels = ap.labels_
...
# X = se.fit_transform(A_A)

# print(n_clusters_)

# # kepo cluster 3d plot GRAPH_b

# fig, ax = plt.subplots(1, 1, subplot_kw={'projection': '3d'}, figsize = (12,8), dpi=100)

# ax.scatter(xs=X[:,0],ys=X[:,1],zs=X[:,2], c=labels, s=100)

# plt.title('Spectral Clustering of GRAPH_b / clusters: 10')
# plt.xlabel('kappa')
# plt.ylabel('kepo')
...
# # kappa spectral clustering GRAPH_b gia 2d plot

# se2_2 = SpectralEmbedding(n_components=2, affinity='precomputed') # n_components = 2d or 3d

# X2_2 = se2_2.fit_transform(B_B)

# # Spectral Clustering 2D plot

# plt.figure(figsize=(12,8), dpi=100)
# plt.scatter(x=X2_2[:,0], y=X2_2[:,1], c=labels2)
...
# Spectral Clustering GRAPH_c - 3D plot

A_c = nx.to_scipy_sparse_matrix(GRAPH_c, weight=None)

sc_c = SpectralClustering(n_clusters=5, assign_labels='kmeans', affinity='precomputed', n_init=100)

labels_c = sc_c.fit_predict(A_c)

se_c = SpectralEmbedding(n_components=3, affinity='precomputed', n_jobs=-1) # n_components = 2d or 3d

X3 = se_c.fit_transform(A_c)

#  Clustering 3D plot GRAPH_c

fig, ax_c = plt.subplots(1, 1, subplot_kw={'projection': '3d'}, figsize = (12,8), dpi=100)

ax_c.scatter(xs=X3[:,0],ys=X3[:,1],zs=X3[:,2], c=labels_c, s=100)

plt.title('Spectral Clustering GRAPH_c / clusters: 5')
plt.xlabel('')
plt.ylabel('')

ax_c.get_xaxis().set_ticks([])
ax_c.get_yaxis().set_ticks([])
ax_c.get_zaxis().set_ticks([])

#plt.savefig(fname=r'/content/drive/MyDrive/Thesis/colabtest1/Spectral_Clustering_3D_GRAPH_c.png', format='png', bbox_inches='tight', pad_inches=0.1, orientation='landscape', transparent=False)

"""# **PROXEIRO_1**"""

avgp = "SELECT graph, number_of_connected_nodes, average_path_length FROM df_average_path_length ;"
cur.execute(avgp)
avgpath = cur.fetchall()

df_avg_path_length = pd.DataFrame(avgpath, columns=['Network','Number of strongly connected nodes','Average Path Length'])

df_avg_path_length

print(df_avg_path_length.to_latex(index=False))

den = "SELECT GRAPH, DENSITY FROM df_density ;"
cur.execute(den)
dens = cur.fetchall()

df_dens = pd.DataFrame(dens, columns=['Network','Density'])

df_dens

print(df_dens.to_latex(index=False))

diam = "SELECT graph, diameter_approximation FROM df_diameter_approximation ;"
cur.execute(diam)
diame = cur.fetchall()
df_diam = pd.DataFrame(diame, columns=['Network','Diameter'])
df_diam

print(df_diam.to_latex(index=False))

mo = "SELECT graph, modularity FROM modularity_of_graphs ;"
cur.execute(mo)
mod = cur.fetchall()
df_mod = pd.DataFrame(mod, columns=['Network','Modularity'])
df_mod

print(df_mod.to_latex(index=False))

tra = "SELECT graph, transitivity FROM df_transitivity ;"
cur.execute(tra)
tran = cur.fetchall()
df_tran = pd.DataFrame(tran, columns=['Network','Transitivity'])
df_tran

print(df_tran.to_latex(index=False))

c = "SELECT GRAPH_c, means FROM df_metric_means_for_GRAPH_c ;"
cur.execute(c)
gc = cur.fetchall()
df_gc = pd.DataFrame(gc, columns=['Metric','Average'])
df_gc

print(df_gc.to_latex(index=False))

sdad = "SELECT GRAPH_d, means FROM df_metric_means_for_GRAPH_d ;"
cur.execute(c)
gdd = cur.fetchall()
df_gdd = pd.DataFrame(gdd, columns=['Metric','Average'])
df_gdd

print(df_gd.to_latex(index=False))

df123 = pd.read_csv('/content/drive/MyDrive/Thesis/colabtest1/1234csv.csv')

df123

print(df123.to_latex(index=False))

"""# **PROXEIRO_2**"""

d1 = np.load('/content/drive/MyDrive/Thesis/colabtest1/modularity_matrix.npy')

g1 = nx.from_numpy_array(d1, create_using = nx.Graph)

print(g1)

nx.is_weighted(g1)

lo1 = nx.community.louvain_communities(g1)
